...
import {
  LayoutDashboard, AppWindow, CreditCard, Bell, User,
  ShoppingBag, Shield, MessageSquare, Receipt, ChevronRight, ArrowUpRight
} from "lucide-react"
...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-[#f5f5f5]">
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex w-72 bg-white border-r border-neutral-200 flex-col h-screen sticky top-0">
        ...
        <nav className="flex-1 px-3 py-6 space-y-1 overflow-y-auto">
          {navItems.map((item) => (
            <Link key={item.href} href={item.href}
              ...
              <item.icon className="w-5 h-5 text-neutral-400 group-hover:text-neutral-700 transition-colors" />
              <span className="flex-1">{item.label}</span>
              <ChevronRight className="w-4 h-4 text-neutral-300 opacity-0 group-hover:opacity-100 transition-all" />
            </Link>
          ))}
          <div className="my-4 px-3"><div className="border-t border-neutral-100" /></div>
          <Link href="/catalog"
            className="flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-medium
              text-neutral-600 hover:text-neutral-900 hover:bg-neutral-50 transition-all duration-200 group">
            <ShoppingBag className="w-5 h-5 text-neutral-400" />
            <span className="flex-1">Каталог</span>
            <ArrowUpRight className="w-4 h-4 text-neutral-300 opacity-0 group-hover:opacity-100 transition-all" />
          </Link>
        </nav>
        ...
        {isAdmin && (
          <div className="p-3 border-t border-neutral-100">
            <Link href="/admin"
              className="flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-medium
                bg-violet-50 text-violet-700 hover:bg-violet-100 transition-all duration-200 group">
        ...
        <div className="p-4 border-t border-neutral-100">
          <div className="flex items-center gap-3 px-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-neutral-900 truncate">{session.user.name || "Пользователь"}</p>
